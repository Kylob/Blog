...
You can access any of these in your Twig templates eg. ``{{ blog.name }}``, including the ``{{ blog.page }}`` you are on.  Eventually this file will be full of authors, categories, and tags that you can easily manage as well.  You can create a category links menu by ``{% set categories = blog.query('categories') %}``, and looping through the array of information.  Other blog queries include '**tags**', '**authors**', '**archives**', '**recent**', '**featured**', '**similar**', '**posts**', and **[...]** listings of every sort, otherwise known as "The Loop".
